...
class ItemScrollController {
  ...
  void printPositions() {
    _scrollableListState!._printPositions();
  }
...
class _ScrollablePositionedListState extends State<ScrollablePositionedList>
  ...
  void initState() {
    ...
    // final correction = _correctIndexAndAlignment(
    //     index: primary.target, alignment: primary.alignment);
    // primary.target = correction[0];
    // primary.alignment = correction[1];
  ...
  void _jumpTo({required int index, required double alignment}) {
    ...
    setState(() {
      // final correction =
      //     _correctIndexAndAlignment(index: index, alignment: alignment);
      // primary.target = correction[0];
      // primary.alignment = correction[1];
      //
  ...
  List<dynamic> _correctIndexAndAlignment(
      {required int index, required double alignment}) {
    final List<ItemPosition> itemPositions =
        primary.itemPositionsNotifier.itemPositions.value.toList();
    if (itemPositions.isNotEmpty) {
      final lastItem = itemPositions
          .firstWhere((item) => item.index == itemPositions.length - 1);
      final firstItem = itemPositions.firstWhere((item) => item.index == 0);
      final chosenItem =
          itemPositions.firstWhere((item) => item.index == index);
      if (widget.reverse) {
        final diff = alignment - chosenItem.itemTrailingEdge;
        if (lastItem.itemTrailingEdge - firstItem.itemLeadingEdge <= 1.0) {
          alignment = 1.0 - lastItem.itemTrailingEdge + firstItem.itemLeadingEdge;
          index = firstItem.index;
        } else if (diff > 0 && firstItem.itemLeadingEdge + diff > 0) {
          alignment = 0;
          index = firstItem.index;
        } else if (diff < 0 && lastItem.itemTrailingEdge + diff < 1.0) {
          alignment = 0;
          index = itemPositions
              .lastWhere((item) =>
                  item.itemLeadingEdge + (lastItem.itemTrailingEdge - 1.0) >= 0)
              .index;
        }
      } else {
        final diff = alignment - chosenItem.itemLeadingEdge;
        if (lastItem.itemTrailingEdge - firstItem.itemLeadingEdge <= 1.0) {
          alignment = 0;
          index = 0;
        } else if (firstItem.itemLeadingEdge + diff > 0 && diff > 0) {
          alignment = -diff;
          index = 0;
        } else if (lastItem.itemTrailingEdge + diff < 1.0 && diff < 0) {
          alignment = 1.0 - lastItem.itemTrailingEdge - diff;
          index = itemPositions
              .firstWhere((item) => item.itemLeadingEdge + diff >= 0)
              .index;
        }
      }
    }
  ...
  void _printPositions() {
    final List<ItemPosition> itemPositions =
        primary.itemPositionsNotifier.itemPositions.value.toList();
    for (final item in itemPositions) {
      print('${item.index}) ${item.itemLeadingEdge}, ${item.itemTrailingEdge}');
    }
  }
